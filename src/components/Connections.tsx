...
function formatConnectionDataItem(
    ...
    const ret = {
        ...
        chains: (chains == null || chains == undefined)?"":chains.reverse().join(' / '),
        rule,
        ...metadata,
...
function renderTableOrPlaceholder(conns: FormattedConn[], apiconfig, toggleIsRefreshPaused) {
    return conns.length > 0 ? (
        <ConnectionTable data={conns} configs={apiconfig} toggleIsRefreshPaused={toggleIsRefreshPaused}/>
    ) : (
...
function Conn({apiConfig}) {
    ...
    const changeIsRefreshPaused = useCallback((value) => {
        setIsRefreshPaused(value);
    }, []);
        ...
        <div>
            ...
            <Tabs>
                ...
                >
                    ...
                    >
                        <TabPanel>
                            <>{renderTableOrPlaceholder(filteredConns, apiConfig, changeIsRefreshPaused)}</>
                            <Fab
                                icon={
                        ...
                        <TabPanel>{renderTableOrPlaceholder(filteredClosedConns, apiConfig, changeIsRefreshPaused)}</TabPanel>
                    </div>
                </div>
