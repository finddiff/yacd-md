...
// import SimpleDialogDemo from "./Dailog";
...
function renderCell(cell: any, apiconfig, toggleIsRefreshPaused) {
    switch (cell.column.id) {
        case 'host':
            return <FormDialog showtext={String(cell.value)} apiconfig={apiconfig} ID={cell.row.original.id} toggleIsRefreshPaused={toggleIsRefreshPaused}/>;
            // return <SimpleDialogDemo showtext={String(cell.value)} apiconfig={apiconfig}/>;
        // return <span onClick={(e) => console.log(e.target.textContent)}>{cell.value}</span>;
        case 'start':
...
function Table({data, configs, toggleIsRefreshPaused}) {
    // console.log(data, configs)
    // const prevApiConfig = useRef(configs);
                        ...
                        {rows.map((row, i) => {
                            ...
                            return row.cells.map((cell, j) => {
                                return (
                                    ...
                                    >
                                        {renderCell(cell, configs, toggleIsRefreshPaused)}
                                    </div>
                                );
                            });
                        })}
