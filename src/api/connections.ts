...
export function fetchData(
    ...
    // ws.addEventListener('error', (event) => (wsState = 3,console.log(event),ws.close()));
...
function newWebSocket(ws: WebSocket, url: string) {
    timer = setTimeout(function () {
        if (wsState == 1) {
            ...
            console.log("connections ws timeout reconect")
            newWebSocket(ws, url);
        }
    }, 1000);
    try {
...
export async function closeConnections(apiConfig: ClashAPIConfig, ID: string) {
    const {url, init} = getURLAndInit(apiConfig);
    return await fetch(url + endpoint + '/' + ID, {...init, method: 'DELETE'});
}
